The first image of the generated dataset is taking from an camera mounted in the center of the windshield and we can see the hood of the car peaking out from the bottom of the image. The image is actually 160 pixels high and 320 pixels wide. The top portion of the image seems to capture hills and trees and the sky which might distract the model more than help it - it makes sense to crop it out of the image. Your model might train faster if you crop each image to focus on only the portion of the image that is useful for predicting a steering angle.
...
# Preprocessing
For normalization, I can add a Lambda layer to the model. Within this Lambda layer, I'll normalize the image by dividing each element by 255 which is the maximum value of an image pixel. 
